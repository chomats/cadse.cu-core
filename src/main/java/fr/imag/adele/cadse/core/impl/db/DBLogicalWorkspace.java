...
//	}
    ...
    public void delete(INamedUUID object) throws CadseException {
		...
		try {
                    DBIteratorID<ID3> ids = _db.getOutgoingLinksAggregation(ID3.class, objId);
	...
	private <T> T toGoodObj(Class<T> clazz, ID3 id) {
		...
		if (ret != null)
			return ret;
		ret = factory.newElt(clazz, id);
		_cache.put(id.objectId, ret);
		return ret;
	}


        public TypeDefinition[] getAllTypes(INamedUUID object) throws CadseException {
		int objId = obj(object);
		try {
        ...
        public void addBindExendsItemType(INamedUUID typeId, ItemType extType) throws CadseException {
    ...
    private INamedUUID load(int objectId) throws ModelVersionDBException {
        ...
        if (typeID == _db.NULL_ID)
            throw new ModelVersionDBException("Cannot found main type of "+objectId);
        int cadseID = _db.getObjectCadse(typeID);
        INamedUUID ret = toGoodObj(INamedUUID.class, new ID3(cadseID, typeID, objectId));
	...
	public CadseRuntime createCadseRuntime(String name, UUID runtimeId, UUID definitionId) {
		CadseRuntime cadseRuntime = new CadseRuntimeImpl(this, name, runtimeId, definitionId);
